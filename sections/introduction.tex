...
Since its introduction in 1983 as a small name system on the ARPA\-NET, the Domain Name System (DNS) 
has grown to 392.5 million registrations across all top-level domains (TLDs)~\cite{dnib_q1_2026}. 
...
Parquet\footnote{\url{https://parquet.apache.org/}} columnar storage format for each day and dataset. \\
...
all data every day, storing a delta, which consists of only the records that changed between measurements, 
could substantially reduce the storage usage. 
...
Since the data is stored in a format where row-group coherence matters, meaning that it is important 
which row and column a value belongs to, it is not possible to store only the new values of each 
...
A full-column delta, however, is impractical for this dataset, as a single row receiving a new value would 
...
in almost no storage benefit. From this it follows that the system needs a decomposition of the database 
...
This paper will first investigate the state-of-the-art in delta-based systems 
...
storage prototype built for the OpenINTEL project\footnote{\url{https://github.com/h0psej0ch/DeltaParq}}. An evaluation framework 
will be presented that assesses both storage and analysis efficiency. With this framework multiple delta strategies will be assessed,
of which the results will be presented. Finally, using the results, conclusions are drawn regarding the practical viability of a 
delta-based storage system for a daily DNS record database.
