...
\hspace{\parindent}\textbf{Infrastructure:} For the design of the DeltaParq system, the structure and resources 
of the current system have been taken into account. The OpenINTEL system 
utilizes a Hive partitioned dataset on an S3 compatible storage cluster~\cite{7460220}.
Furthermore, Apache Spark, with its Python derivative PySpark, is used for 
querying and analyzing the dataset. Consequently, the DeltaParq is based on PySpark 
and supports storage retrieval from an S3 cluster. However, while the DeltaParq system 
has been designed for the OpenINTEL project, it has also been designed with 
...
\textbf{Self-Describing Storage:} Like the current OpenINTEL system, DeltaParq stores data using a Hive-style path on an S3 
bucket. DeltaParq constructs the path using the TLD, date, and S3 bucket that has been set 
in the configuration.
\[
\begin{gathered}
    \texttt{"s3a://\{bucket\}/tld=\{TLD\}/year=\{year:04\}}\\
    \texttt{/month=\{month:02\}/day=\{day:02\}/"}
\end{gathered}
\]
...
\textbf{Storage Strategies:} Since a delta will always be based on a single base dataframe, a storage strategy which determines 
...
\textbf{Schema Decomposition:} As mentioned in the problem statement, the OpenINTEL dataset 
...
The resulting volatile dataframe can immediately be written to storage.
For the non-volatile data all original columns, excluding the columns marked as volatile, 
but including the primary key, are selected. This dataframe then needs to be processed 
using delta generation, after which it can be written to storage.

\textbf{Delta Generation:} The concept of delta generation is based on filtering out all the rows that are different 
in the updated version compared to the base version. The OpenINTEL dataset, and DNS records 
in general, have an inherent flaw that makes this difficult: DNS records do not have a unique identifier.
...
For the DeltaParq, the latter option of storing the full identified block was chosen based on the average block size 
...
\textbf{Dependency Resolution:} Given a delta strategy function\\ $strategy: \mathbb{Z} \rightarrow \mathbb{Z}$ that maps each
...
days are requested, the resulting dependency chains are merged together without duplicates.

\textbf{Delta Reconstruction:} Using the resolved dependency chain, the full dataframe for each day in the chain can be reconstructed 
...
in the update cannot be present in the delta dataframe. The volatile dataframe measures all the present blocks and 
...
\textbf{Public Interface:} The DeltaParq offers both a $get()$ and $get\_list()$ interface for retrieving data. With $get\_list()$ the retrieval 
...
\label{sec:performance_constraints}
...
and retrieval part of the DeltaParq use various joins and subtracts which are wide operations. In the case of spark executing 
on multiple executors, the executors need to share information for these wide operations. An executor may have a row that needs to be joined with a row 
...
when writing to storage, and PySpark were able to read this, the now broad execution path of combining all the data can be transformed into a direct linear 
